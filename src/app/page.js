...
export default async function HomePage() {
  ...
  const [hero, featured] = await Promise.all([
    getArtwork(HERO_SLUG),
    getArtworks(FEATURED_SLUGS),
  ]);
  ...
  return (
    <>
      ...
      <Section
        eyebrow="À propos"
        title="Un musée qui tient dans un onglet"
        className="bg-paper-2"
      >
        <div className="grid gap-12 md:grid-cols-2">
          <p className="text-lead text-ink-2">
            Le Métropolitain rassemble une sélection resserrée de chefs-d'œuvre
            incontournables, du Primitif flamand au Cubisme. Chaque œuvre
            dispose de sa fiche, de son mouvement et de ses œuvres voisines.
          </p>
          <div className="space-y-6">
            <p className="text-lead text-ink-2">
              Les données proviennent de l'API Museum, une collection mise à
              jour régulièrement.
            </p>
