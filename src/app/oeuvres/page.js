...
import { filterArtworks, getAllArtworks, getMovements, getTypes } from '@/lib/museum'

export const metadata = {
  title: 'Les œuvres',
  description: 'Parcourez la collection : filtrez par département et période, et ouvrez chaque œuvre en détail.',
...
export default async function ArtworksPage({ searchParams }) {
  ...
  const [artworks, movements, types] = await Promise.all([getAllArtworks(), getMovements(), getTypes()])
  ...
  return (
    <>
      <section className='pb-10 pt-24'>
        <Container>
          <div>
            <p className='cartel mb-6 text-stone'>{query ? `Recherche : « ${query} »` : null}</p>
            <div className='flex flex-wrap items-baseline gap-4'>
              <h1 className='text-title'>{query ? 'Résultats' : 'Toutes les œuvres'}</h1>
      ...
      <Container>
        ...
        <Suspense fallback={<div className='h-32 border-y border-line' />}>
          <ArtworkFilters artworks={artworks} movements={movements} types={types} />
        </Suspense>
      </Container>
    </>
  )
