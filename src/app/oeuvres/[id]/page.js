...
export async function generateMetadata({ params }) {
  ...
  const description =
    ...
    [artwork.artist, artwork.date, artwork.movement].filter(Boolean).join(' · ')
...
export default async function ArtworkPage({ params }) {
  ...
  return (
    <>
      <article className='pt-24'>
        <Container className='grid gap-14 lg:grid-cols-12'>
          <div className='lg:col-span-7'>
            <div className='relative flex aspect-[4/3] items-center justify-center bg-paper-2 lg:sticky lg:top-28'>
              {artwork.imageLarge ? (
                <Image
                ...
                <span className='cartel text-stone'>Image non disponible</span>
              )}
            </div>
          </div>
      ...
      {related.length > 0 && (
        <Section eyebrow='Dans la même veine' title='À découvrir ensuite' className='mt-24'>
